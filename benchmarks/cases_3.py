...
CASES = (
    Case(
        ...
        expected=(RiskSpec('async_lifecycle', ('app/notify.py',), ('error_propagation',)),),
        hard=False,
    ),
    Case(
    ...
    Case(
        ...
        expected=(RiskSpec('cache_consistency', ('app/profile.py',), ('stale_state',)),),
        hard=False,
    ),
    Case(
        id='12-indirect-dependency',
        title='Indirect caller assumes old error behavior',
        ticket='Allow repository lookup failures to propagate so API callers can choose their response, while existing background label generation must remain tolerant of missing keys.',
        ...
        expected=(RiskSpec('error_propagation', ('app/service.py', 'app/scheduler.py'), ('indirect_dependency',)),),
        hard=True,
    ),
)
